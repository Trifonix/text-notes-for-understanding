def main(arg: int) -> str:  # Контракт функции main*
    print("рвавшаоа")
    pass  # заглушка
    return "сообщение отправлено"


#  Что такое декларативное и императивное написание программы или метод решения задач? *
#  Императивное - попунктное решение проблемы, декларативное - при решении отталкивание не от конкретных подпунктов, а одновременное решение общей проблемы.
...
class MyTransport:  # (Декларирование класса MyTransport) объявление класса
    on_air: bool  # Поле класса  (используется обычно при соединение например с базой данных через класс. *)
    on_earth: bool
    # Инкапсуляция (на собесе объясняя это понятие - отталкивайся от термина. (incapsulate что в переводе капсулирование - означает капсулирование полей класса внутри самого класса либо допуск к их изменению в случае public полей.))
    material_type_korpus: str  # public
    _material_type_salon: str  # private - запрещен нестрого для использования вне класса, используется только для изменения методов класса либо (есть тех возможность) при изменении атрибутов класса.
    __material_type_engine: str  # protected - тоже самое что privatе только строгое.
    DATABASE_URL: str = None
    
    def __init__(self):  # Конструктор класса  / метод класса
        # self.is_active = is_active  # Атрибут класса
        pass
